...
const Register = () => {
  ...
  const validateForm = () => {
    ...
    if (!rollNumber.trim()) {
      toast({ title: "Validation Error", description: "Roll number is required", variant: "destructive" });
      return false;
    }
    if (!department.trim()) {
  ...
  const handleSendOTP = async () => {
    ...
    try {
      ...
      toast({ title: "OTP Sent Successfully!", description: `Verification code sent to ${formData.email}. Check your email inbox.` });
    } catch (error: any) {
      toast({ title: "Failed to send OTP", description: error.message || "Network error", variant: "destructive" });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleVerifyOTP = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!otp || otp.length !== 6) {
      toast({ title: "OTP Required", description: "Please enter the complete 6-digit OTP", variant: "destructive" });
      return;
    }
    if (!/^\d{6}$/.test(otp)) {
      toast({ title: "Invalid OTP", description: "OTP must contain only numbers", variant: "destructive" });
      return;
    }
    setLoading(true);
    try {
      ...
      toast({ title: "Registration Successful! 🎉", description: "Welcome to OG Techminds!" });
      setFormData({ fullName: '', email: '', phone: '', rollNumber: '', department: '', year: '', password: '', confirmPassword: '' });
      setOtp(''); setOtpSent(false);
      setTimeout(() => { navigate("/login"); }, 2000);
    } catch (error: any) {
      toast({ title: "Verification Failed", description: error.message || "Check OTP & try again.", variant: "destructive" });
    } finally {
      setLoading(false);
    }
  };

  const handleBackToRegistration = () => { setOtpSent(false); setOtp(''); setResendCooldown(0); };
  ...
  const handleGoogleSuccess = async (credentialResponse: any) => {
    if (credentialResponse.credential) {
      try {
        ...
        toast({ title: "Google Login Successful!", description: `Welcome ${data.name || ""}` });
        navigate("/dashboard");
      } catch (err: any) {
      ...
      <Card className="w-full max-w-2xl bg-black/40 border-purple-500/20 backdrop-blur-md">
        <CardHeader className="text-center">
          ...
          <CardDescription className="text-gray-300">
            {!otpSent ? 'Create your account to become a member' : `Enter the OTP sent to ${formData.email}`}
          </CardDescription>
        </CardHeader>
        <CardContent>
          {!otpSent ? (
            <>
              <div className="space-y-4">
                ...
                <Button onClick={handleSendOTP} className="w-full bg-purple-600 hover:bg-purple-700 text-white" disabled={loading}>
              ...
              <div className="mt-6 flex justify-center">
                <GoogleLogin
                  ...
                  onError={() => toast({ title: "Google Login Failed", description: "Please try again.", variant: "destructive" })}
                />
              </div>
            </>
          ) : (
            <form onSubmit={handleVerifyOTP} className="space-y-6">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  disabled={loading}
                />
                <p className="text-sm text-gray-400 text-center">Check your email inbox and spam folder</p>
              </div>

              <Button type="submit" className="w-full bg-purple-600 hover:bg-purple-700 text-white" disabled={loading || otp.length !== 6}>
                {loading ? (<><Loader2 className="mr-2 h-4 w-4 animate-spin" />Verifying...</>) : 'Verify & Register'}
              </Button>

              <div className="flex space-x-4">
                <Button type="button" variant="outline" className="flex-1 bg-purple-600 hover:bg-purple-700 text-white" onClick={handleBackToRegistration} disabled={loading}>Back</Button>
                <Button type="button" variant="outline" className="flex-1 bg-purple-600 hover:bg-purple-700 text-white" onClick={handleResendOTP} disabled={loading || resendCooldown > 0}>
          ...
          <div className="mt-6 text-center">
            <p className="text-gray-300">
              ...
              <Link to="/login" className="text-purple-400 hover:text-purple-300 underline">Sign in</Link>
            </p>
          </div>
        </CardContent>
      </Card>
